...
//    }
    ...
    public DRGWSResult InsertDRGResult(final DataSource datasource,
        ...
        try (Connection connection = datasource.getConnection()) {
            ...
            CallableStatement grouperdata = connection.prepareCall("call MINOSUN.DRGPKGPROCEDURE.INSERT_DRG_RESULT(:Message,:Code,:rest_id,:series,:tags,:lhio,:pdx,:sdx,:proc)");
...
//            GetPDx.setString("AgeMax", MaxAge);
    ...
    public DRGWSResult MDC(final DataSource datasource, final String mdc) {
        ...
        MDC mdcData = new MDC();
        try (Connection connection = datasource.getConnection()) {
            CallableStatement GetMDC = connection.prepareCall("begin :mdc_output := MINOSUN.DRGPKGFUNCTION.GET_MDC(:mdcs); end;");
