...
//                    }
    ...
    public DRGWSResult UpdateDRGResult(final DataSource datasource,
        ...
        result.setSuccess(false);
        try (Connection connection = datasource.getConnection()) {
            CallableStatement updatedrgresult = connection.prepareCall("call DRG_SHADOWBILLING.DRGPKGPROCEDURE.UPDATE_DRG_RESULT(:Message,:Code,:umdc,:updc,:udc,:uresultid,:useries,:utags,:udrg,:drgdetails)");
...
// GET ICD10 VALIDATION WITH PARAMETER
    ...
    public DRGWSResult UnralatedANDORProc(final DataSource datasource, final String icd9codes, final String mdccode) {
        ...
        result.setSuccess(false);
        try (Connection connection = datasource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :unralated_or_proc := DRG_SHADOWBILLING.DRGPKGFUNCTION.GET_UNRALATED_PROC_ORPROC(:icd9codes,:mdccode); end;");
    ...
    public DRGWSResult AgeConfictValidation(final DataSource datasource,
        ...
        result.setSuccess(false);
        try (Connection connection = datasource.getConnection()) {
            CallableStatement state = connection.prepareCall("begin :age_validation := DRG_SHADOWBILLING.DRGPKGFUNCTION.VALIDATE_AGE(:p_pdx_code,:age_day,:age_min_year); end;");
    ...
    public DRGWSResult AX(final DataSource datasource, final String axcodes, final String requestcode) {
        ...
        result.setSuccess(false);
        try (Connection connection = datasource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :get_ax := DRG_SHADOWBILLING.DRGPKGFUNCTION.GET_AX_PARAM(:axcodes); end;");
