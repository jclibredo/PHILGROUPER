...
//                    DRGWSResult pdcresult = new GetPDC().GetPDC(datasource, mdcProcedure.getA_PDC().trim(), drgResult.getMDC());
                ...
                if (checkAX.AX(datasource, "2PCX", ProcedureList.get(y).trim()).isSuccess()) {
                    Counter2PCX++;
                }
            ...
            int CounterPDx2BX = 0;
            if (checkAX.AX(datasource, "2BX", grouperparameter.getPdx()).isSuccess()) {
                CounterPDx2BX++;
            }
            if (new PDxMalignancy().PDxMalignancy(datasource, grouperparameter.getPdx(), "2E".trim()).isSuccess()) {
                MalignantCount++;
            }
                ...
                if (utility.isValidDCList(drgResult.getDC())) {
                    ...
                    if (getpcclvalue.isSuccess()) {
                        ...
                        if (new DRG().DRG(datasource, drgResult.getDC(), finaldrgresult.getDRG()).isSuccess()) {
                            ...
                            if (new ValidatePCCL().ValidatePCCL(datasource, drgResult.getDC(), finaldrgresult.getDRG()).isSuccess()) {
                                drgResult.setDRG(drgResult.getDC() + new ValidatePCCL().ValidatePCCL(datasource, drgResult.getDC(), finaldrgresult.getDRG()).getResult());
