...
public class GetMDC28 {
    ...
    public DRGWSResult GetMDC28(
        ...
        try {
            ...
            CaCRx += CaCRxSDx > 0 && CaCRxProc > 0 ? 1 : 0;
            Counter28EX += checkAX.AX(datasource, SchemaName, "28EX", grouperparameter.getPdx().toUpperCase().trim()).isSuccess() ? 1 : 0;
            Counter28BX += checkAX.AX(datasource, SchemaName, "28BX", grouperparameter.getPdx().toUpperCase().trim()).isSuccess() ? 1 : 0;
            Counter28CX += checkAX.AX(datasource, SchemaName, "28CX", grouperparameter.getPdx().toUpperCase().trim()).isSuccess() ? 1 : 0;
            // PROCESS BEGINS HERE
