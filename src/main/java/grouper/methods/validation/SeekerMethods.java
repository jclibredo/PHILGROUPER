...
public class SeekerMethods {
    ...
    public DRGWSResult GETTOKEN(
        ...
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :v_result := " + SchemaName + ".drgseeker.GETTOKEN(); end;");
    ...
    public DRGWSResult InsertToken(
        ...
        result.setSuccess(false);
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("call " + SchemaName + ".drgseeker.TOKEN_S(:message,:code,:ptoken,:pdatecreated)");
    ...
    public DRGWSResult UserInsert(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            if (this.GetUserByUsername(dataSource, SchemaName, seekerUser.getEmail()).isSuccess()) {
                ...
                CallableStatement statement = connection.prepareCall("call " + SchemaName + ".drgseeker.insertuser(:message,:code,:pemail,:ppassword,:prole,:udatecreated,:ucreatedby,:ustatus,:uname)");
                statement.registerOutParameter("Message", OracleTypes.VARCHAR);
                statement.registerOutParameter("Code", OracleTypes.INTEGER);
                ...
                if (statement.getString("Message").equals("SUCC")) {
                    result.setSuccess(true);
                    result.setMessage(statement.getString("Message"));
                    this.EmailSender(dataSource, SchemaName, seekerUser.getEmail().trim(), seekerUser.getPassword(), mailsession, "ACCOUNT");
                } else {
                    result.setMessage(statement.getString("Message"));
                }
            }
    ...
    public DRGWSResult GetUserByID(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :v_result := " + SchemaName + ".drgseeker.GETUSERBYID(:puserid); end;");
            statement.registerOutParameter("v_result", OracleTypes.CURSOR);
    ...
    public DRGWSResult GetUserByUsername(
            final DataSource dataSource,
            final String SchemaName,
            final String pusername) {
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :v_result := " + SchemaName + ".drgseeker.GETUSERBYUSERNAME(:pusername); end;");
            statement.registerOutParameter("v_result", OracleTypes.CURSOR);
            ...
            if (resultset.next()) {
                ...
                if (resultset.getString("UPDATEDBY") == null || resultset.getString("UPDATEDBY").isEmpty() || resultset.getString("UPDATEDBY").equals("")) {
                    ...
                    if (this.GetUserByID(dataSource, SchemaName, resultset.getString("UPDATEDBY").trim()).isSuccess()) {
                        SeekerUser getuser = utility.objectMapper().readValue(this.GetUserByID(dataSource, SchemaName, resultset.getString("UPDATEDBY")).getResult(), SeekerUser.class);
                ...
                if (resultset.getString("CREATEDBY") == null || resultset.getString("CREATEDBY").isEmpty() || resultset.getString("CREATEDBY").equals("")) {
                    ...
                    if (this.GetUserByID(dataSource, SchemaName, resultset.getString("CREATEDBY").trim()).isSuccess()) {
                        SeekerUser getuser = utility.objectMapper().readValue(this.GetUserByID(dataSource, SchemaName, resultset.getString("CREATEDBY")).getResult(), SeekerUser.class);
    ...
    public DRGWSResult GetAllUser(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :v_result := " + SchemaName + ".drgseeker.GETALLUSER(); end;");
            statement.registerOutParameter("v_result", OracleTypes.CURSOR);
            statement.execute();
    ...
    public DRGWSResult UserLogin(
        ...
        try {
            DRGWSResult getUserDetails = this.GetUserByUsername(dataSource, SchemaName, uemail.trim());
...
//                                        result.setMessage("DAYS FIRST OTP CODE IS EXPIRED " + Long.parseLong(this.GetDatesDifferential(userA.getOtpdatecreated(), "DAYS")));
    ...
    public DRGWSResult ValidateUserUpdate(
            final DataSource dataSource,
            final String SchemaName,
            final SeekerUser seekerUser) {
        DRGWSResult result = utility.DRGWSResult();
        try {
            if (!seekerUser.getUserid().isEmpty()) {
                DRGWSResult getUserbyid = this.GetUserByID(dataSource, SchemaName, seekerUser.getUserid());
                if (getUserbyid.isSuccess()) {
                    ...
                    if (user.getEmail().trim().equals(seekerUser.getEmail().trim())) {
                        ...
                        DRGWSResult getCount = this.COUNTEMAIL(dataSource, SchemaName, seekerUser.getEmail());
    ...
    public DRGWSResult UserUpdate(
            final DataSource dataSource,
            final String SchemaName,
            final SeekerUser seekerUser) {
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("call " + SchemaName + ".drgseeker.edituser(:message,:code,:pemail,:ppassword,:prole,:puserid,:ustatus,:uname,:udateupdated,:uupdatedby)");
            statement.registerOutParameter("Message", OracleTypes.VARCHAR);
            statement.registerOutParameter("Code", OracleTypes.INTEGER);
            statement.setString("pemail", seekerUser.getEmail().trim());
    ...
    public DRGWSResult COUNTEMAIL(
            final DataSource dataSource,
            final String SchemaName,
            final String pusername) {
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("begin :v_result := " + SchemaName + ".drgseeker.COUNTEMAIL(:pusername); end;");
            statement.registerOutParameter("v_result", OracleTypes.CURSOR);
            statement.setString("pusername", pusername.trim());
            statement.execute();
            ResultSet resultset = (ResultSet) statement.getObject("v_result");
    ...
    public DRGWSResult ForgatPassword(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try {
            DRGWSResult getAccountID = this.GetUserByUsername(dataSource, SchemaName, uemail);
            if (getAccountID.isSuccess()) {
                SeekerUser user = utility.objectMapper().readValue(getAccountID.getResult(), SeekerUser.class);
                DRGWSResult updatepassword = this.UPDATEPASSWORD(dataSource, SchemaName, user.getUserid(), uemail, randpass);
    ...
    public DRGWSResult UPDATEPASSWORD(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("call " + SchemaName + ".drgseeker.UPDATEPASSWORD(:message,:code,:puserid,:pemail,:ppasswordd)");
            statement.registerOutParameter("Message", OracleTypes.VARCHAR);
            statement.registerOutParameter("Code", OracleTypes.INTEGER);
            statement.setString("puserid", puserid.trim());
    ...
    public DRGWSResult TestEmailSender(
        ...
        try {
            ...
            DRGWSResult validateUsername = this.GetUserByUsername(dataSource, SchemaName, emailreciever);
            if (!validateUsername.isSuccess()) {
                ...
                if (randpass.length() > 0) {
                    ...
                    if (type.trim().toUpperCase().equals("OTP")) {
                        ...
                        DRGWSResult updatepassword = this.UPDATEPASSWORD(dataSource, SchemaName, user.getUserid(), emailreciever, randpass);
                        if (updatepassword.isSuccess()) {
                            result.setSuccess(true);
                    ...
                    if (type.trim().toUpperCase().equals("OTP")) {
                        ...
                        DRGWSResult updatepassword = this.UPDATEPASSWORD(dataSource, SchemaName, user.getUserid(), emailreciever, newPass);
                        if (updatepassword.isSuccess()) {
                            result.setSuccess(true);
    ...
    public DRGWSResult POSTOTP(
        ...
        DRGWSResult result = utility.DRGWSResult();
        try (Connection connection = dataSource.getConnection()) {
            CallableStatement statement = connection.prepareCall("call " + SchemaName + ".drgseeker.POSTOTP(:message,:code,:puserid,:potp,:udatecreated)");
            statement.registerOutParameter("Message", OracleTypes.VARCHAR);
            statement.registerOutParameter("Code", OracleTypes.INTEGER);
            statement.setString("puserid", puserid.trim());
